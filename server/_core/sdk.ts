...
class SDKServer {
   ...
   * Returns the raw session token to be stored in the client cookie.
   */
  async createDbSession(
    userId: number,
    openId: string,
    ipAddress?: string,
    userAgent?: string
  ...
  /**
   * Verify a raw session token from a cookie.
   * Returns the associated user if valid, otherwise throws ForbiddenError.
   */
  async verifyDbSession(rawToken: string | undefined | null): Promise<User> {
    if (!rawToken) {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    return this.verifyDbSession(sessionCookie);
  }
}
