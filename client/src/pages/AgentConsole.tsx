...
import { Bot, Send, Zap, CheckCircle2, AlertCircle, Clock, ChevronDown, ChevronUp } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { toast } from "sonner";

const AGENT_ROLES = [
  { id: "orchestrator", label: "Orchestrator", desc: "Parses intent, maintains context" },
...
export default function AgentConsole() {
  ...
  const { data: runs, isLoading, refetch } = trpc.agentRuns.list.useQuery(
    { orgId },
      ...
      <div className="space-y-6 animate-fade-up">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-black/40 mt-1">Multi-agent security intelligence orchestration</p>
          </div>
        ...
        <div className="rounded-2xl border border-black/8 bg-white p-5">
          <h2 className="font-semibold text-sm mb-4 text-black/60 uppercase tracking-wider text-xs">Agent Pipeline</h2>
          <div className="flex items-center gap-1 overflow-x-auto pb-2">
            {AGENT_ROLES.map((agent, i) => (
              <div key={agent.id} className="flex items-center gap-1 shrink-0">
                <div className="text-center">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-1.5 mx-auto"
                    style={{ background: "oklab(0.7 0.0260473 -0.147721 / 0.12)" }}>
                    <Bot className="w-4 h-4 text-black/60" />
                  </div>
                  <div className="text-xs font-semibold text-black/70 whitespace-nowrap">{agent.label}</div>
                  <div className="text-xs text-black/30 whitespace-nowrap max-w-20 truncate">{agent.desc}</div>
                </div>
        ...
        {/* Input */}
        <form onSubmit={handleSubmit} className="rounded-2xl border border-black/8 bg-white p-5">
        ...
        {/* Run History */}
        <div>
          <h2 className="font-semibold text-sm mb-3 text-black/60">Run History</h2>
          {isLoading ? (
            <div className="space-y-3">
              {[...Array(3)].map((_, i) => <div key={i} className="h-20 bg-black/5 rounded-2xl animate-pulse" />)}
            </div>
          ) : !runs?.length ? (
            <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl border border-dashed border-black/10">
              ...
              <p className="text-sm text-black/40">No agent runs yet. Ask the agent something above.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {runs.map(run => {
                const isExpanded = expandedRunId === run.id;
                const trace = Array.isArray(run.agentTrace) ? run.agentTrace as Array<{agent: string; action: string; timestamp: string}> : [];
                return (
                  <div key={run.id} className="rounded-2xl border border-black/8 bg-white overflow-hidden">
                    <button
                      onClick={() => setExpandedRunId(isExpanded ? null : run.id)}
                      className="w-full flex items-center gap-4 p-4 text-left hover:bg-black/2 transition-colors">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${
                        run.status === "completed" ? "" : run.status === "failed" ? "bg-black" : "bg-black/10"
                      }`}
                        style={run.status === "completed" ? { background: "oklab(0.7 0.0260473 -0.147721 / 0.2)" } : {}}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium truncate">{run.intent}</div>
                        <div className="flex items-center gap-3 mt-0.5">
                          <span className="text-xs text-black/30">{new Date(run.createdAt).toLocaleString()}</span>
                          {run.durationMs && (
                            <span className="text-xs text-black/30">{(run.durationMs / 1000).toFixed(1)}s</span>
                          )}
                          {run.tokensUsed && (
                            <span className="text-xs text-black/30">{run.tokensUsed} tokens</span>
                          )}
                        </div>
                      </div>
                      {isExpanded ? <ChevronUp className="w-4 h-4 text-black/30 shrink-0" /> : <ChevronDown className="w-4 h-4 text-black/30 shrink-0" />}
                    </button>
                      ...
                      <div className="px-4 pb-4 border-t border-black/5 pt-4 space-y-3">
                        ...
                        {trace.length > 0 && (
                          <div className="space-y-2">
                            <div className="text-xs font-semibold text-black/40 uppercase tracking-wider">Agent Trace</div>
                            {trace.map((step, i) => (
                              <div key={i} className="flex items-start gap-3 text-xs">
                                ...
                                <div>
                                  <span className="font-semibold text-black/70">{step.agent}</span>
                                  <span className="text-black/40 ml-2">{step.action}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        {run.result && (
                          <div className="rounded-xl p-4 text-sm leading-relaxed"
                            ...
                            <div className="text-xs font-semibold text-black/40 mb-2">Result</div>
                            <div className="text-black/70 whitespace-pre-wrap">{run.result}</div>
                          </div>
                        )}
                      </div>
                    )}
