...
export default function Devices() {
  ...
  const atRiskCount =
    devices?.filter(d => d.status !== "secure" && d.status !== "unknown")
      .length ?? 0;
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-black/40 mt-1">
              {devices?.length ?? 0} monitored · {secureCount} secure ·{" "}
              {atRiskCount} at risk
            </p>
          </div>
          <Button
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <Button
                    variant="outline"
                    onClick={() => setShowAdd(false)}
                    className="flex-1 rounded-xl border-black/15 h-10"
                  >
                    Cancel
                  </Button>
                  <Button
                    onClick={handleAdd}
                    disabled={!form.name.trim() || createDevice.isPending}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-black/40 mb-6">
              Add your first device to start monitoring for vulnerabilities.
            </p>
            <Button
              onClick={() => setShowAdd(true)}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {devices.map(device => {
              ...
              return (
                <div
                  key={device.id}
                  className="rounded-2xl border border-black/8 bg-white p-5 hover:border-black/15 transition-all group"
                >
                  ...
                  <div className="space-y-1.5 text-xs text-black/40">
                    {device.firmwareVersion && (
                      <div className="flex items-center justify-between">
                        <span>Firmware</span>
                        <span className="font-mono font-medium text-black/60">
                          {device.firmwareVersion}
                        </span>
                      </div>
                    )}
          ...
          <div className="rounded-2xl border border-dashed border-black/10 p-5 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-black/40 leading-relaxed">
                Sentinel automatically maps your devices to NVD CPE identifiers
                using exact, fuzzy, and fingerprint matching strategies. Devices
                with manufacturer and model information have higher match
                confidence.
              </p>
            </div>
          </div>
