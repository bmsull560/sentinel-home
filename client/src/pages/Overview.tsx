      ...
      <div className="space-y-8 animate-fade-up">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-black/40 mt-1">
              Monitor your digital security posture and respond to threats
            </p>
          </div>
          <div
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {metrics.map(({ label, value, sub, icon: Icon, urgent }) => (
            <div
              key={label}
              className="rounded-2xl p-5 border border-black/8 bg-white hover:border-black/15 transition-all"
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center ${urgent ? "bg-black" : ""}`}
                  style={
                    urgent
                      ? {}
                      : { background: "oklab(0.7 0.0260473 -0.147721 / 0.15)" }
                  }
                >
                  <Icon
                    className={`w-4 h-4 ${urgent ? "text-white" : "text-black"}`}
                  />
                </div>
              </div>
              <div className="text-3xl font-bold tracking-tight">{value}</div>
              <div
                className={`text-xs mt-1 font-medium ${urgent ? "text-black" : "text-black/40"}`}
              >
                {sub}
              </div>
            </div>
          ...
          <div className="lg:col-span-2 rounded-2xl border border-black/8 bg-white p-6">
            ...
            {!vulns?.length ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <div className="text-xs text-black/25 mt-1">
              ...
              <div className="space-y-3">
                {vulns.slice(0, 5).map(v => {
                  const cfg =
                    SEVERITY_CONFIG[v.severity] ?? SEVERITY_CONFIG.calm;
                  return (
                    <div
                      key={v.id}
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full shrink-0 ${
                          v.severity === "immediate_attention"
                            ? "bg-black"
                            : v.severity === "action_recommended"
                              ? "bg-black/60"
                              : v.severity === "be_aware"
                                ? "bg-black/30"
                                : "bg-black/15"
                      ...
                      <span
                        className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${cfg.bg} ${cfg.text}`}
                      >
                        {cfg.label}
                      </span>
                    </div>
              ...
              <div className="relative w-32 h-32">
                <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                  <circle
                    ...
                    r="15.9"
                    fill="none"
                    stroke="oklab(0.7 0.0260473 -0.147721 / 0.1)"
                    strokeWidth="3"
                  />
                  <circle
                    cx="18"
              ...
              {[
                {
                  label: "Secure",
                  count: secureCount,
                  color: "oklab(0.7 0.0260473 -0.147721 / 0.5)",
                },
                { label: "At Risk", count: atRiskCount, color: "black" },
                {
                  label: "Unknown",
                  count: total - secureCount - atRiskCount,
        ...
        {(data?.monitoredDevices ?? 0) === 0 && (
          <div className="rounded-2xl border border-dashed border-black/15 p-12 text-center">
            ...
            <p className="text-sm text-black/40 mb-6 max-w-sm mx-auto">
              Add your connected devices to start monitoring for vulnerabilities
