...
const DEVICE_CATEGORIES = [
  { id: "smart_home", label: "Smart Home", desc: "Locks, cameras, thermostats" },
...
export default function Onboarding() {
  ...
  const [orgName, setOrgName] = useState(user?.name ? `${user.name}'s Home` : "My Home");
  ...
  const handleOrgName = (v: string) => {
    setOrgName(v);
    setOrgSlug(v.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 64));
  };
      ...
      <div className="flex items-center gap-2 mb-10">
        {[1, 2].map(s => (
          <div key={s} className="flex items-center gap-2">
            ...
            {s < 2 && <div className={`w-12 h-px transition-all ${step > s ? "bg-black" : "bg-black/15"}`} />}
          </div>
        ))}
      </div>
        ...
        {step === 1 && (
          <div className="animate-fade-up">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold tracking-tight mb-2">Name your workspace</h1>
              <p className="text-black/50 text-sm">This is your organization in Sentinel. You can change it later.</p>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-black/50 uppercase tracking-wider block mb-1.5">Workspace name</label>
                <Input
                  value={orgName}
              ...
              <div>
                <label className="text-xs font-semibold text-black/50 uppercase tracking-wider block mb-1.5">URL slug</label>
                <div className="flex items-center gap-0 border border-black/15 rounded-xl overflow-hidden h-12">
                  <span className="px-3 text-sm text-black/40 bg-black/5 h-full flex items-center border-r border-black/10">sentinel.app/</span>
                  <Input
                    value={orgSlug}
        ...
        {step === 2 && (
          <div className="animate-fade-up">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold tracking-tight mb-2">What devices do you have?</h1>
              <p className="text-black/50 text-sm">Select all that apply. You can add specific devices later.</p>
            </div>
              ...
              {DEVICE_CATEGORIES.map(({ id, label, desc }) => {
                ...
                return (
                  <button
                    ...
                    <div className={`text-xs ${selected ? "text-white/60" : "text-black/40"}`}>{desc}</div>
                  </button>
                );
              })}
            </div>
            ...
            <button onClick={() => setLocation("/dashboard")} className="w-full text-center text-sm text-black/40 hover:text-black mt-3 transition-colors">
              Skip for now
            </button>
          </div>
        )}
